...
class MapDetailViewController: UIViewController, MKMapViewDelegate, CLLocationManagerDelegate, UITableViewDelegate, UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        let eachResponse = responseResult[indexPath.row]
        self.mapData = eachResponse
        viewLocation()
        let lat = eachResponse.placemark.location?.coordinate.latitude
        let long = eachResponse.placemark.location?.coordinate.longitude
    ...
    @IBAction func doneBtnPressed(_ sender: Any) {
         performSegue(withIdentifier: "mapToChat", sender: self)
    }
...
extension MapDetailViewController: HandleMapSearch {
    func dropPinZoomIn(placemark:MKPlacemark){
        // cache the pin
